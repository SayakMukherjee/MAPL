...
class DatasetWrapper(datasets.SVHN):
    ...
    def __init__(self, 
                 ...
                 download: bool = False):
        super(DatasetWrapper, self).__init__(root, split, transform, target_transform, download)

        self.classes = np.unique(np.array(self.labels)).tolist()
...
class SVHN(BaseDataset):
    ...
    def __init__(self, config: Config, client_id: int, isSSL: bool = False, dataset_args: dict = None):
        ...
        self.data_path = Path(self.config.settings['configurations']['data_path']).joinpath('svhn')
    ...
    def __init_train_dataset(self):
        ...
        self.logger.info(f"Loading SVHN train data")

        if self.transform is None:
            self.train_dataset = DatasetWrapper(root = self.data_path, split = 'train', download = True)
        else:
            self.train_dataset = DatasetWrapper(root = self.data_path, split = 'train', download = True, transform = self.transform)    
    ...
    def __init_test_dataset(self):
        ...
        self.logger.info(f"Loading SVHN test data")
        
        self.test_dataset = DatasetWrapper(root=self.data_path, split='test', download=True, transform=self.test_transform)
