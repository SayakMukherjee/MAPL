...
class DatasetWrapper(datasets.ImageFolder):
    ...
    def __init__(self, 
        ...
        super(DatasetWrapper, self).__init__(root, transform, target_transform, loader, is_valid_file)
...
class CINIC10(BaseDataset):
    ...
    def __init__(self, config: Config, client_id: int, isSSL: bool = False, dataset_args: dict = None):
        super(CINIC10, self).__init__(config)
        
        self.logger = logging.getLogger(self.__class__.__name__)

        self.data_path = Path(self.config.settings['configurations']['data_path']).joinpath('cinic10')
    ...
    def __init_train_dataset(self):
        ...
        self.logger.info(f"Loading CINIC10 train data")

        train_dir = self.data_path.joinpath('train')

        if self.transform is None:
            self.train_dataset = DatasetWrapper(root = train_dir)
        else:
            self.train_dataset = DatasetWrapper(root = train_dir, transform = self.transform)    
    ...
    def __init_test_dataset(self):
        ...
        self.logger.info(f"Loading CINIC10 test data")

        test_dir = self.data_path.joinpath('test')
        
        self.test_dataset = DatasetWrapper(root=test_dir, transform=self.test_transform)
